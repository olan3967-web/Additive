...
async function loadSigninPage() {
    ...
    container.innerHTML = `
        <div class="card">
            <h3><i class="fas fa-calendar-alt"></idata-i18n=" 每日签到奖励"> 每日签到奖励</h3>
            <button id="addSigninDayBtn" class="success" style="margin-bottom: 20px;"><i class="fas fa-plus"></idata-i18n=" Add签到日"> Add签到日</button>
    ...
    if (div) {
        ...
        for (let r of rewards || []) {
            div.innerHTML += `<div style="background:#0f172a; border-radius:16px; padding:15px; margin-bottom:12px; display:flex; align-items:center; gap:15px; flex-wrap:wrap;">
                ...
                <button class="saveReward" data-day="${r.day}" style="background:#2f6b3a; padding:6px 16px; border-radius:8px;"data-i18n="Save"data-i18n="Save">Save</button>
                <button class="deleteReward" data-day="${r.day}" style="background:#7a2f2f; padding:6px 16px; border-radius:8px;"data-i18n="Delete"data-i18n="Delete">Delete</button>
            </div>`;
        }
